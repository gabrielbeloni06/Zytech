...
import { Check, Star, X, MessageSquare, Mail, Workflow, ArrowRight } from 'lucide-react';
...
export const ServiceCard = ({ icon, title, desc, active, onClick, tag, delay }) => {
  return (
    <div 
      ...
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-600/10 to-purple-600/10 rounded-bl-full -mr-10 -mt-10 transition-all group-hover:scale-150"></div>
      
      <div className="relative z-10 mb-auto">
        <div className="mb-6 p-4 bg-white/5 w-fit rounded-2xl border border-white/10 group-hover:scale-110 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 text-gray-400">
          {icon}
        </div>
...
export const ContactModal = ({ plan, onClose }) => {
  ...
  return (
    <div onClick={handleBackdropClick} className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/90 backdrop-blur-md animate-fade-in p-4">
      <div className="bg-slate-900 border border-white/10 rounded-[2rem] p-8 max-w-md w-full relative shadow-2xl animate-fade-in-up overflow-hidden">
        ...
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg text-white">
            <MessageSquare size={32} />
          </div>
          <h2 className="text-2xl font-bold text-white mb-2">Excelente Escolha!</h2>
          <p className="text-gray-400 text-sm">Você está a um passo de contratar o plano <strong className="text-white">{plan}</strong>.</p>
        </div>

        <div className="space-y-3">
          <a href={`https://wa.me/553180209584?text=Olá,%20tenho%20interesse%20no%20plano%20${plan}%20da%20Zytech`} target="_blank" rel="noopener noreferrer" 
            className="flex items-center justify-center gap-3 w-full py-4 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl transition-all shadow-lg shadow-green-900/20 group">
